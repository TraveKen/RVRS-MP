...
class Slider(Widget, can_focus=True):
    ...
    class Changed(Message):
        ...
        def control(self) -> Slider:
            return self.slider
    class MouseCapture(Message):
        def __init__(self, *args):
            super().__init__()
            self.args = args

    class MouseRelease(Message):
        def __init__(self, *args):
            super().__init__()
            self.args = args

    class SongEnd(Message):
        def __init__(self, *args):
            super().__init__()
            self.args = args
    def __init__(
    ...
    def move_slider(self) -> None:
        if self.value == self.max:
            self.post_message(self.SongEnd(self))
        else:
            if self._grabbed == None and self.state == 'playing':
    ...
    def _on_mouse_capture(self, event: events.MouseCapture) -> None:
        ...
        self.post_message(self.MouseCapture(self))

    def _on_mouse_release(self, event: events.MouseRelease) -> None:
        self._grabbed = None
        event.stop()
        self.post_message(self.MouseRelease(self))
    ...
    """def moveslider(self):
        while True:
            self.action_slide_right(self)
            sleep(1)

    def control(self, status):
        self.moveslider_thread = Process(target=self.moveslider, args=(self,))
        self.moveslider_thread.start()"""

    def on_mount(self) -> None:
        #Event handler called when widget is added to the app.
        self.set_interval(1/1, self.move_slider)
    ...
    def set_state(self, state):
        self.state = state
        self.refresh()
    def update(self,value: int, max: int):
        self.max = max
        self.value = value
        self.refresh()